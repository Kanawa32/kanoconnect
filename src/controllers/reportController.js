...
export const exportReport = asyncHandler(async (req, res) => {
  ...
  let csv = '';
  const filename = `report-${type || 'shipments'}-${startDate}-${endDate}.csv`;
  ...
  if (type === 'revenue') {
    ...
    data.forEach(r => { csv += `${r._id},${r.revenue},${r.count}\n`; });
  } else {
    const data = await Shipment.find(matchStage)
      .populate('customer', 'firstName lastName')
      .populate('rider', 'firstName lastName')
      .sort('-createdAt')
      .lean();
    csv = 'Tracking Number,Customer,Rider,Pickup,Delivery,Status,Total Amount,Rider Earnings,Created\n';
    data.forEach(s => {
      const row = [
        s.trackingNumber,
        s.customer ? `${s.customer.firstName} ${s.customer.lastName}` : '',
        s.rider ? `${s.rider.firstName} ${s.rider.lastName}` : '',
